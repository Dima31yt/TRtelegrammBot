...
exit_keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
buttons = ["Выход в главное меню"]
exit_keyboard.add(*buttons)

bot.send_message(1356201546, "bot started")
bot.send_message(1024861899, "bot started")
...
def get_text_messages(message):
    try:
        ...
        if message.text.lower() == 'общая область':
            ...
            general_keyboard.add(telebot.types.KeyboardButton("Беседы Точек Роста"))
        ...
        if message.text.lower() == 'обратная связь':
            bot.send_message(message.chat.id, "У вас есть вопрос или Вы хотите предложить новый сервис?\nНапишите в нашу техническую поддержку:\nhttps://clck.ru/UMxhp\n\nБот разработан обучающимися центра \"Точка Роста\" МБОУ \"Первомайская СОШ\" Первомайского района Тамбовмкой области.\n\nАктивное участие в разработке принимали: @maaaasyyyniiiik, @sviridoff52\n\nСайт центра: tochkarosta.pervsosh.ru", reply_markup=exit_keyboard)
        ...
        if message.text.lower() == 'беседы точек роста':
            bot.send_message(  
                message.chat.id,  
